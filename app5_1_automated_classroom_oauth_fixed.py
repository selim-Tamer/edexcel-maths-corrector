...
with st.expander("🎓 Google Classroom — automatic marking & grade return", expanded=True):
    ...
    if not classroom_auth_configured():
        st.warning(
            "Google Classroom is not configured yet. Add your Google OAuth "
            "client ID, client secret, and redirect URI under [auth] and "
            "[auth.google] in Streamlit Community Cloud Secrets."
        )
        st.caption(
            "The normal PDF marker below will still work with GEMINI_API_KEY."
        )
    else:
        if st.session_state.get("classroom_oauth_error"):
            st.error(st.session_state.pop("classroom_oauth_error"))

        auth_col1, auth_col2 = st.columns([3, 1])
        with auth_col1:
            if google_logged_in():
                st.success("✅ Google Classroom API access is connected.")
            else:
                st.info(
                    "Authorize Google Classroom access. This uses Google's "
                    "OAuth authorization flow for the Classroom and Drive APIs."
                )
        with auth_col2:
            if google_logged_in():
                ...
                try:
                    ...
                    st.error(f"Could not start Google authorization: {auth_error}")
